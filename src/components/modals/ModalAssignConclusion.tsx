...
const ModalSendForEDSConclusion: React.FC<Props> = ({ close, open, conclusionId }) => {
    ...
    const onSubmit = (comment: any) => {
        ...
        ConclusionsServices.assignConclusion(conclusionId, formdata)
        ...
        <>
            <Portal close={close} open={open}>
                <form onSubmit={handleSubmit(onSubmit)} className='form-item modal__form modal__eds'>
                    <h3>{t("assign_conclusion_by_eds")}</h3>
...
export default ModalSendForEDSConclusion