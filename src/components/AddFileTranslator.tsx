...
import { AddTranslatorFile } from '../constants/validation'
...
const AddFileTranslatorForm: React.FC<PropsForm> = ({ conclusionId, name }) => {
  ...
  const { register, handleSubmit, control, reset, formState: { errors } } = useForm({ mode: 'onBlur', resolver: yupResolver(AddTranslatorFile) })
...
const AddFileTranslator: React.FC<Props> = ({ conclusionId }) => {
  const { t, i18n } = useTranslation('common')
  ...
  return (
    <>
      ...
      <div className={conclusionLang == 1 ? "item-details__content--tab item-details__uploads-kk" : "item-details__content--tab item-details__uploads-ru"}>
        {conclusionLang == 1 ? <AddFileTranslatorForm name={"translater_document_src_kk"} conclusionId={conclusionId} /> : null}
        {conclusionLang == 2 ? <AddFileTranslatorForm name={"translater_document_src_ru"} conclusionId={conclusionId} /> : null}
      </div>
    </>
  )
}

export default AddFileTranslator